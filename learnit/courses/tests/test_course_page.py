...
class CategoryPageTest(TestCase):
    def setUp(self):
        ...
        self.category = Category.objects.create(name="Programming", slug="programming", language=self.language)
    ...
    def test_category_page_status_code(self):
        response = self.client.get(reverse("category", kwargs={"category": self.category.slug}))
    ...
    def test_breadcrumbs_in_context(self):
        ...
        self.assertEqual(response.context["breadcrumbs"][-1]["name"], self.category.name)

    def test_404_on_nonexistent_category(self):
        response = self.client.get(reverse("category", kwargs={"category": "nonexistent-category"}))
